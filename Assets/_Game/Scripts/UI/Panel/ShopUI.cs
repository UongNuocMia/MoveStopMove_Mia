...
public class ShopUI : UICanvas
{
    ...
    private void OnChangeItem(ShopItem.OnItemClickEventArgs args)
    {
        if (currentShopItem != null)
            currentShopItem.IsClickThisItem(false);
        ...
        bool isInList = false;
        currentShopItem.IsClickThisItem(true);
    ...
    private void SetCoin()
    {
        ...
        if (UserDataManager.Ins.GetCoin() < price)
            coinText.SetText($"<color=red>{currentCoin}</color>");
        else
            coinText.SetText($"<color=#FFF500>{currentCoin}</color>");
    }
